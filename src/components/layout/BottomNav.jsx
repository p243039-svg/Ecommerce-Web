...
export function BottomNav() {
    ...
    return (<nav className="fixed bottom-0 inset-x-0 bg-background/80 backdrop-blur-2xl border-t border-border/50 z-50 sm:hidden flex items-center justify-around h-20 px-2 pb-safe">
      {navItems.map((item) => {
            ...
            return (<Link key={item.label} href={item.href} className={cn("relative flex flex-col items-center justify-center gap-1.5 w-16 h-16 transition-all duration-300", isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
